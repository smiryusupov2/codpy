...
def mnist_data(num = 1):
    ...
    return get_matrix(x_vals).T, get_matrix(marg1), get_matrix(marg2), dist_ref, X, y, num
...
def stats_d(empirical, samples):
    ...
    cov = pd.DataFrame(cov_difference[:2,:2], index=['Empirical', 'Sampled'], columns =['Empirical', 'Sampled'])
    return quant, cov
...
class cond_marginal_plot:
    ...
    def graphic(self,dists):
        ...
        def fun_plot(params,**kwargs):
            ...
            sns.histplot(data=data, x=col,hue="class", bins=100,cumulative = True,ax=ax,stat="density", common_norm=False,fill=False,element="step")
            title = col+ "|"+ cond_col
            ax.set_title(title)
            ax.legend()
        multi_plot(self.get_list(dists),mp_ncols=len_,mp_nrows=len(dists),fun_plot=fun_plot)
...
def iris_example(**kwargs):
    return cond_marginal_plot(iris_data,threshold = 0.25)(samplers = [codpy.conditioning.ConditionerKernel,codpy.conditioning.NadarayaWatsonKernel],**kwargs)
