...
def NWkernel_density_estimator(
    x,
    y,
    kernel=None,
    **kwargs
...
class ConditionerKernel(Kernel):
    def __init__(self, x,y, latent_x = None, latent_y = None,**kwargs):
        ...
        if latent_y is None:
            self.latentd_y = lambda N: np.random.normal(size=[N,self.get_fx().shape[1]])
        else:
        ...
        self.map_xy_inv = Kernel(x=self.map_xy.get_fx(),fx = self.map_xy.get_x(), **kwargs)
