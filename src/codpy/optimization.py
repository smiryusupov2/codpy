...
def continuous_optimizer(
    function, distribution, n=1000, n_iter=5, contracting_factor=0.3, maximize=True,include=None,**kwargs
):
    if maximize == False:
        return continuous_optimizer(
            lambda x: -function(x)-function.distance(x), distribution, n, n_iter, contracting_factor, True ,**kwargs
        )
...
def grad_optimizer(
    function, distribution, n=1000, n_iter=5, contracting_factor=0.3, maximize=True,**kwargs
):
    extremum_y, extremum_x = continuous_optimizer(function, distribution, n, n_iter, contracting_factor, maximize,**kwargs)
    if hasattr(function,"grad"):
        ...
        delta = (f(1e-9) - f(0.))/1e-9
        xmin, fval, iter, funcalls = optimize.brent(
            f, brack=(0.0, 1.), maxiter=5, full_output=True
        )
